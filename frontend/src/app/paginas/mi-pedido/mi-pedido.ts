...
confirmarPedido() {
 ...
 const detalles = this.itemsDelCarrito.map((item: any) => {
    ...
    personalizacion: item.personalizacion
      ? {
          ...
          costoAdicional: item.personalizacion.costoAdicional != null 
            ...
            : null,
          adicionalesSeleccionados: item.personalizacion.adicionalesSeleccionados || []
        }
      : null
  };
});
  ...
  this.http.post<any>('http://localhost:8080/api/pedidos/confirmar', request, { headers })
    .subscribe({
      next: (res) => {
      ...
      error: (err) => {
        ...
        alert('Error: ' + (err.error?.message || 'No se pudo guardar el pedido'));
      }
    });
}
}